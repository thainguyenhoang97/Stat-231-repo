...
gdp_cons <- gapminder %>%
  filter(year == 2007) %>%
  mutate(cat = gdpPercap%/%10000) %>%
  select(country,gdpPercap,cat)

world_map_2 <- left_join(world_map,gdp_cons, by = c("region" = "country"))
#summary(gdp_cons)
...
ggplot(world_map_2, aes(x = long, y = lat, group = group
  ...
  theme_void() +
  coord_fixed(ratio = 1.3) +
  labs(title = "GDP per capita of countries in the world - (done by T Nguyen)"
       , subtitle = "as of 2007"
       , caption = "* States in grey have no data"
       , fill = "") +
  scale_fill_viridis(option = "magma", direction = -1) 

...
> ANSWER: 
1) Can you code so that there is no reactivity? For example if the user interacts with various inputs and you don't want the graph in the output to change until the user commits to all their inputs changes, is that possible? (wrote this before I reach isolate() but still curious about the multiple inputs but only isolate a few :D)
2) Why must reactive objects be recalled as a function later on?
3) What is the goal of passing things through the html() function?

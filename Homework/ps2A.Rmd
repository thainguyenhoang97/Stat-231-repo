...
> ANSWER: This is called narrow because there are only 2 columns of variables but many rows of observations. It is also easier for us to add in variables if needs be.
(I have a question on this exercise, because the code chunk on page 102 makes the data "wide" with the "spread" function, which in the context of the book means separating the sexes into Male and Female. So if we follow the book closely then the data would be wide and not narrow? Couldn't find the time to clarify on this so I am putting it here)
...
> ANSWER: Because when we widen the data we only keep observations where both 'M' and 'F' are higher than 10000. Thus when we narrow it back down there are only observations that satisfy those criteria being narrowed down back to sex and total. 

...
> RESPONSE: The file has 2 columns and 13 rows. The variables are "away", "bullseye", "circle", "dino", "dots", "h_lines", "high_lines", "slant_down", "slant_up", "star", "v_lines", "wide_lines" and "x_shape".
...
> RESPONSE: The correlation coefficient is -0.06447185. This tells me that x and y has a negative correlation in the dataset, and to be precise, a 1 unit increase of x leads to a 0.06447185 unit decrease of y.
...
> RESPONSE:It seems like x and y combines to give a plot of a dinosaur...
...
> RESPONSE: The correlation coefficient is -0.0629611, which is almost the same as that from the 'dino' dataset.

```{r}
# Start with the `datasaurus_dozen` and pipe it into the `filter` function to filter for observations where `dataset == "star"`. Store the resulting filtered data frame as a new data frame called `star_data`.
...
> RESPONSE: The plot is not the same, because now it plots a star instead. I feel like I see where this is going...
...
> RESPONSE: The correlation coefficient is -0.06834336, once again very similar to that of the two previous exercises. 

```{r}
# Start with the `datasaurus_dozen` and pipe it into the `filter` function to filter for observations where `dataset == "circle"`. Store the resulting filtered data frame as a new data frame called `circle_data`.
...
> RESPONSE: The plot doesn't look similar to the other 2 datasets, but it draws a circle. I however saw 3 points that lie rather out of the circumference. 
...
What do you notice?  Is the correlation coefficient an appropriate summary for any of these datasets?  Why or why not?

> RESPONSE: Once again, the correlation coefficient between x and y from all the different data sets are very close to one another, but are not an appropriate summary of these datasets as the plots show that these graphs don't really have a linear relationship between x and y but rather just plot points for a shape of some sort.
